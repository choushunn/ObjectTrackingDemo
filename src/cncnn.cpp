...
int CNcnn::detectYolov5(const cv::Mat& bgr, std::vector<Object>& objects)
{
...
void CNcnn::drawObjects(const cv::Mat& bgr, const std::vector<Object>& objects)
{
    ...
    for (size_t i = 0; i < objects.size(); i++)
    {
        ...
        char text[256];
        sprintf(text, "%s %.1f%%", m_classNames[obj.label], obj.prob * 100);
    ...
    QImage new_image = cvMatToQImage(image);
